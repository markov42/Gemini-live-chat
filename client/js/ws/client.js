...
export class GeminiWebsocketClient {
    ...
    async receive(blob) {
        try {
            ...
            console.log(`${this.name} received response:`, JSON.stringify(response).substring(0, 200) + "...");
            ...
            if (response.serverContent) {
                ...
                if (serverContent.modelTurn) {
                    ...
                    console.log(`${this.name} model turn, parts count:`, parts.length);
                    ...
                    console.log(`${this.name} audio parts:`, audioParts.length);
    ...
    async sendAudio(base64audio) {
        ...
        console.log(`Sending audio chunk to ${this.name}, size: ${base64audio.length} chars`);
        try {
    ...
    async sendJSON(json) {        
        try {
            ...
            if (this.ws && this.ws.readyState === WebSocket.OPEN) {
                ...
                console.warn(`Cannot send message to ${this.name}: WebSocket is not in OPEN state (current state: ${this.ws ? this.ws.readyState : 'null'})`);
                
                // If the connection is closed/closing, try to reconnect
                if (!this.ws || this.ws.readyState === WebSocket.CLOSED) {
                    ...
                    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
                        ...
                        throw new Error(`Failed to reconnect WebSocket for ${this.name}`);
                    }
                }
            }
        } catch (error) {
            console.error(`Failed to send ${JSON.stringify(json)} to ${this.name}:`, error);
            throw new Error(`Failed to send message to ${this.name}: ${error.message}`);
        }
    }
}